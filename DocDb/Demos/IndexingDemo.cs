...
namespace DocDb.DotNetSdk.Demos
{
	public static class IndexingDemo
	{
		...
		private async static Task AutomaticIndexing(DocumentClient client)
		{
			...
			//var collection = await client.CreateDocumentCollectionAsync("dbs/FabianToDoList", collectionDefinition);
            var collection = await client.CreateDocumentCollectionAsync("dbs/FabianToDoList", collectionDefinition);
			...
			Document indexedDocument = await client
				.CreateDocumentAsync("dbs/FabianToDoList/colls/autoindexing", indexedDocumentDefinition);
			...
			Document unindexedDocument = await client
				.CreateDocumentAsync(
                    "dbs/FabianToDoList/colls/autoindexing",
			...
			var doeDocs = client.CreateDocumentQuery("dbs/FabianToDoList/colls/autoindexing", "SELECT * FROM c WHERE c.lastName = 'Doe'").ToList();
			...
			var allDocs = client.CreateDocumentQuery("dbs/FabianToDoList/colls/autoindexing", "SELECT * FROM c").ToList();
			...
			Document janeDoc = client
				.CreateDocumentQuery("dbs/FabianToDoList/colls/autoindexing", "SELECT * FROM c WHERE c.id = 'JANE'")
				.AsEnumerable()
				.FirstOrDefault();
			...
			await client.DeleteDocumentCollectionAsync("dbs/FabianToDoList/colls/autoindexing");
		}
		...
		private async static Task ManualIndexing(DocumentClient client)
		{
			...
			Document unindexedDocument = await client
				.CreateDocumentAsync("dbs/FabianToDoList/colls/manualindexing", unindexedDocumentDefinition);
			...
			Document indexedDocument = await client
				.CreateDocumentAsync(
					"dbs/FabianToDoList/colls/manualindexing",
					indexedDocumentDefinition,
					new RequestOptions { IndexingDirective = IndexingDirective.Include });

			// Unindexed document won't get returned when querying on non-ID (or self-link) property
			var doeDocs = client.CreateDocumentQuery("dbs/FabianToDoList/colls/manualindexing", "SELECT * FROM c WHERE c.lastName = 'Doe'").ToList();
			Console.WriteLine("Documents WHERE lastName = 'Doe': {0}", doeDocs.Count);

			// Unindexed document will get returned when using no WHERE clause
			var allDocs = client.CreateDocumentQuery("dbs/FabianToDoList/colls/manualindexing", "SELECT * FROM c").ToList();
			Console.WriteLine("All documents: {0}", allDocs.Count);

			// Unindexed document will get returned when querying by ID (or self-link) property
			Document johnDoc = client
				.CreateDocumentQuery("dbs/FabianToDoList/colls/manualindexing", "SELECT * FROM c WHERE c.id = 'JOHN'")
				.AsEnumerable()
				.FirstOrDefault();

			Console.WriteLine("Unindexed document self-link: {0}", johnDoc.SelfLink);

			await client.DeleteDocumentCollectionAsync("dbs/FabianToDoList/colls/manualindexing");
		}
		...
		private async static Task SetIndexPaths(DocumentClient client)
		{
			...
			Document doc1 = await client.CreateDocumentAsync("dbs/FabianToDoList/colls/customindexing", doc1Definition);
			...
			Document doc2 = await client.CreateDocumentAsync("dbs/FabianToDoList/colls/customindexing", doc2Definition);
			...
			Document doc3 = await client.CreateDocumentAsync("dbs/FabianToDoList/colls/customindexing", doc3Definition);

			// Works with range index on title property strings
			var byTitle = client.CreateDocumentQuery("dbs/FabianToDoList/colls/customindexing", "SELECT * FROM c ORDER BY c.title").ToList();

			// Doesn't works without range index on category property strings (returns 0 documents, but doesn't throw error!)
			var byCategory = client.CreateDocumentQuery("dbs/FabianToDoList/colls/customindexing", "SELECT * FROM c ORDER BY c.category").ToList();

			// Works with range index on rank property numbers
			var tryRankSort = client.CreateDocumentQuery("dbs/FabianToDoList/colls/customindexing", "SELECT * FROM c ORDER BY c.rank").ToList();

			// Delete the collection
			await client.DeleteDocumentCollectionAsync("dbs/FabianToDoList/colls/customindexing");
		}
