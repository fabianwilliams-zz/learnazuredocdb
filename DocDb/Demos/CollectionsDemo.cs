	...
	public static class CollectionsDemo
	{
		...
		public async static Task Run()
		{
			...
			using (var client = new DocumentClient(new Uri(endpoint), masterKey))
			{
                ...
                _database = client.CreateDatabaseQuery("SELECT * FROM c WHERE c.id = 'FabianToDoList'").AsEnumerable().First();
		...
		private async static Task CreateCollection(DocumentClient client, string collectionId, string offerType = "S1")
		{
			...
			Console.WriteLine("Created new collection with Range Indexing Policy");
			ViewCollection(collection);
		}
