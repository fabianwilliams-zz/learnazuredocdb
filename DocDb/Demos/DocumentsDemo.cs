	...
	public static class DocumentsDemo
	{
		...
		public async static Task Run()
		{
			...
			using (var client = new DocumentClient(new Uri(endpoint), masterKey))
			{
                ...
                Database database = client.CreateDatabaseQuery("SELECT * FROM c WHERE c.id = 'FabianToDoList'").AsEnumerable().First();

                _collection = client.CreateDocumentCollectionQuery(database.CollectionsLink, "SELECT * FROM c WHERE c.id = 'MyCollection1'").AsEnumerable().First();
